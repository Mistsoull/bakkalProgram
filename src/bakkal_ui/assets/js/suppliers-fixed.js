...
function initializeDataTable() {
    ...
    try {
        ...
        suppliersDataTable = $(suppliersTable).DataTable({
            ...
            order: [[0, 'asc']], // Firma adına göre alfabetik sırala
            columnDefs: [
                {
...
function createSupplierRowData(supplier) {
    ...
    const contactInfo = [];
    if (supplier.phoneNumber) contactInfo.push(`<i class="fas fa-phone text-primary me-1"></i>${escapeHtml(supplier.phoneNumber)}`);
    if (supplier.nameSurname) contactInfo.push(`<i class="fas fa-user text-secondary me-1"></i>${escapeHtml(supplier.nameSurname)}`);
    
    const contactDisplay = contactInfo.length > 0 ? contactInfo.join('<br>') : '<span class="text-muted">-</span>';
    ...
    return [
        ...
        `<div class="d-flex align-items-center">
            ...
            <div>
                ...
                ${supplier.nameSurname ? `<small class="text-muted">${escapeHtml(supplier.nameSurname)}</small>` : ''}
            </div>
        </div>`,
        
        // İletişim
        `<div class="fs-3">${contactDisplay}</div>`,
